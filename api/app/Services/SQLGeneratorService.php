...
class SQLGeneratorService
{
    ...
    public function queryChatGPTForSQL($userQuery, $schema)
    {
        ...
        $prompt = "You are an EXPERT WordPress developer and SQL developer. Your task is to analyze the database schema and generate a CORRECT MySQL SELECT query.\n\n" .
                  ...
                  "    → JOIN orders table with customers/users table\n" .
                  "    → Use SELECT to show customer info (name, email) and order info (order_id, order_date, total)\n" .
                  "    → Use ORDER BY order_date DESC to show most recent first\n" .
                  "    → Use LIMIT N for 'last N customers' or LIMIT 50 for 'customers list'\n" .
                  "    → Example: SELECT users.display_name, users.user_email, orders.order_id, orders.date_created FROM wc_orders AS orders JOIN wp_users AS users ON orders.customer_id = users.ID ORDER BY orders.date_created DESC LIMIT 10\n" .
                  "    → Check schema for EXACT table names: might be wc_orders + wp_users, or wc_order_stats + wp_users\n" .
                  "    → Check schema for EXACT column names: customer_id might be customer_id, user_id, or billing_email\n" .
                  "    → Check schema for EXACT date column: might be date_created, date_created_gmt, order_date, post_date\n" .
                  ...
                  "⚠️⚠️⚠️ CRITICAL: TABLE NAMES IN SCHEMA ARE EXACT - USE THEM EXACTLY ⚠️⚠️⚠️\n" .
